...
async def get_all_accounts(offset:int = Query(0, ge=0), limit:int = Query(20, le=100), admin = Depends(get_current_admin_user)):
...
async def get_deleted_accounts(offset:int = Query(0, ge=0), limit:int = Query(20, le=100), admin = Depends(get_current_admin_user)):

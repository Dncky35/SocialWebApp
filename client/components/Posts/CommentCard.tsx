...
const CommentCard: React.FC<CommentProps> = ({ comment }) => {
    ...
    return (
        <div className={`bg-gradient-to-br ${isOwnComment ? "from-slate-600 to-slate-800" : "from-slate-700 to-slate-900"} 
        rounded shadow-xl px-4 py-2 hover:scale-[1.01] transform transition duration-300 w-full`}>
            <div className='border-b border-slate-700 py-1 mb-1 flex items-center justify-between'>
                <Link href={`/profile/${comment.author_id}`} className='font-semibold text-lg text-slate-300 cursor-pointer hover:underline'>
            ...
            <div className="flex items-center justify-between border-t border-slate-700 py-2">
                ...
                <p className='text-sm text-slate-300'>Posted On: {new Date(comment.created_at).toLocaleDateString()}</p>
            </div>
